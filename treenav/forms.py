...
class MenuItemFormMixin(object):
        ...
        if any([link.startswith(s) for s in ("http://", "https://")]):
            ...
            try:
                ...
                raise forms.ValidationError(
                    "Please supply a valid URL! , URL " "name, or regular expression."
                )
    ...
    def clean(self):
        ...
        if (
            "is_enabled" in self.cleaned_data
            and self.cleaned_data["is_enabled"]
            and "link" in self.cleaned_data
            and self.cleaned_data["link"].startswith("^")
        ):
            raise forms.ValidationError(
                "Menu items with regular expression " "URLs must be disabled."
            )
...
class GenericInlineMenuItemForm(forms.ModelForm):
    parent = TreeNodeChoiceField(queryset=MenuItem.objects.all(), required=False)
